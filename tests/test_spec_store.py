...
{{
    ...
    def test_set_header(self):
        ...
        with patch.object(self.store, "specChanged") as mock_signal:
            ...
            assert (
                self.store.spec["default_headers"]["Content-Type"] == "application/json"
            )
            mock_signal.emit.assert_called_once()
    ...
    def test_remove_all_headers(self):
        """Test removing all headers"""
        # Add multiple headers
        self.store.spec["default_headers"] = {
        ...
        with patch.object(self.store, "specChanged") as mock_signal:
            ...
            # Should reset to default Accept header only
            assert self.store.spec["default_headers"] == {"Accept": "application/json"}
            mock_signal.emit.assert_called_once()

    def test_remove_all_headers_when_empty(self):
        """Test removing all headers when already empty"""
        self.store.spec["default_headers"] = {}

        with patch.object(self.store, "specChanged") as mock_signal:
            self.store.remove_all_headers()

            # Should reset to default Accept header
            assert self.store.spec["default_headers"] == {"Accept": "application/json"}
            mock_signal.emit.assert_called_once()

    def test_remove_all_headers_preserves_only_accept(self):
        """Test that remove_all_headers preserves the Accept header"""
        # Add custom headers including Accept
        self.store.spec["default_headers"] = {
            "Accept": "application/xml",  # Custom Accept value
            "Content-Type": "application/json",
            "Authorization": "Bearer token",
        }
        ...
        with patch.object(self.store, "specChanged") as mock_signal:
            ...
            # Should reset Accept to default value
            assert self.store.spec["default_headers"] == {"Accept": "application/json"}
            mock_signal.emit.assert_called_once()
...
admin/settings"""
    ...
    def test_set_endpoints(self):
        ...
        endpoints = [("Users List", "GET", "/users"), ("Create User", "POST", "/users")]
        ...
        self.store.spec["endpoints"] = [
            {
                ...
                "method": "GET",
                "path": "/test",
                "expect": {"admin": {"status": 200}, "guest": {"status": 403}},
            }
        ]
    ...
    def test_export_as_postman_with_original_data(self):
        ...
        original_postman = {
            ...
            "auth": {
                "type": "bearer",
                "bearer": [{"key": "token", "value": "old-token"}],
            },
            "item": [],
        }
...
class TestSpecStoreEdgeCases:
    ...
    def test_extract_base_url_with_various_formats(self):
        ...
        test_cases = [
            ...
            (
                {"item": [{"request": {"url": "http://127.0.0.1:3000/test"}}]},
                "http://127.0.0.1:3000",
            ),
        ]
    ...
    def test_role_operations_with_special_characters(self):
        ...
        assert success
        assert (
            self.store.spec["roles"]["long-token-user"]["auth"]["token"] == long_token
        )
